...
import { CATEGORIES, PRODUCTS } from "../utils/products.js";
...
export default function Productos() {
  ...
  const list = useMemo(() => {
    let items = PRODUCTS;
    if (categoria !== "todos") items = items.filter((p) => p.category === categoria);
    if (query.trim()) {
      const q = query.toLowerCase();
      items = items.filter((p) => p.name.toLowerCase().includes(q) || p.features.some((f) => f.toLowerCase().includes(q)));
    }
    return items;
  }, [categoria, query]);
